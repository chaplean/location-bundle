 ...
 * @since     6.0.1
 */
class LoadDepartmentDataTest extends MockeryTestCase
{
    /**
    ...
    public function testLoad109Departments()
    {
        ...
        $objectManager->shouldReceive('persist')->times(109);
        $objectManager->shouldReceive('flush')->once();

        /**
         * @var $loadDepartmentData \PHPUnit_Framework_MockObject_MockObject|LoadDepartmentData
         */
        $loadDepartmentData = $this->getMockBuilder(LoadDepartmentData::class)->setMethods(['getReference', 'setReference'])->getMock();
        $loadDepartmentData->expects($this->any())->method('getReference');
        $loadDepartmentData->expects($this->any())->method('setReference');
